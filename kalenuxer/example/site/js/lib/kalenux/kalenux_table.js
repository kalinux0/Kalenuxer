...
function setNavs(table) {
    ...
    c = Math.ceil(table.count / table.limit);
    if (table.start > table.count) {
...
function setNav(a, table) {
    ...
    switch (a) {
        case -1:
            ...
            break;
        case -2:
            if (table.start < table.limit) {
                table.start = 0;
                break;
            }
            table.start -= table.limit;
            e = Math.ceil(table.start / table.limit) + 1;
            break;
        case -3:
            e = Math.ceil((table.start + table.limit) / table.limit) + 1;
            if (e > c) {
                return;
            }
            table.start += table.limit;
            break;
        case -4:
            table.start = Math.floor(b) * table.limit;
            e = Math.ceil(table.start / table.limit) + 1;
            if (table.page == e) {
                return;
            }
...
function setTables() {
    ...
    function setTable(a) {
        ...
        a = document.getElementsByClassName('table-filter');
        b = a.length;
        for (i = 0; i < b; i++) {
            c = a[i];
            d = c.dataset;
            e = d.type;
            if (e === 'search') {
                c.onkeydown = tableSearch.bind(null, tables[n]);
            } else if (e === 'date') {
                $('#' + c.id).datepicker({
                    onSelect: updateTable.bind(null, tables[n])
                });
            } else {
                c.onchange = updateTable.bind(null, tables[n]);
            }
        }
...
function updateTable(table) {
    ...
    table.data.start = table.start;
    table.data.limit = table.limit;
    ...
    postJSON(c, function(a, b) {
        if (a.result === 1) {
            ...
            console.log(d);
            if (d == 0) {
                document.getElementsByClassName('kalenux-table-holder')[0].dataset.state = "hidden";
                return;
            } else {
                document.getElementsByClassName('kalenux-table-holder')[0].dataset.state = "";
            }
            if (!d) {
                d = 1;
            }
